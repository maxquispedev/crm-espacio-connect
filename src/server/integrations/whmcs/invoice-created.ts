...
export class WhmcsIntegrationError extends Error {
  status: number;
  code: string;

  constructor(status: number, code: string, message: string) {
    super(message);
    this.name = "WhmcsIntegrationError";
    this.status = status;
    this.code = code;
  }
}
...
export type InvoiceCreatedDeps = {
...
export async function reserveEvent(input: {
  organizationId: string;
  externalId: string;
}): Promise<ReservedEvent> {
  const db = getDb();
  ...
  const existing = await db
    ...
    .where(
      scoped(
        ...
        eq(schema.integrationEvent.eventType, INVOICE_CREATED_EVENT),
...
export async function markEvent(input: {
  organizationId: string;
  id: string;
  status: "completed" | "failed";
  messageId?: string | null;
  ...
  const db = getDb();
  await db
    .update(schema.integrationEvent)
    .set({
      status: input.status,
    ...
    .where(
      and(
        eq(schema.integrationEvent.id, input.id),
        scoped(schema.integrationEvent.organizationId, input.organizationId)
      )
    );
}

/**
 * Elige `invoice_created` approved del tenant. Preferencia de idioma:
 * `es_MX`, luego `es`, luego la más antigua. Un único name+status puede
 * existir en varios idiomas (unique es org+name+language).
 */
export async function findApprovedInvoiceTemplate(
  organizationId: string
): Promise<{ id: string } | null> {
  const db = getDb();
  const rows = await db
    ...
    .where(
      scoped(
        ...
        organizationId,
        eq(schema.template.name, INVOICE_CREATED_TEMPLATE),
...
export const defaultInvoiceCreatedDeps: InvoiceCreatedDeps = {
  findOrgBySlug,
  reserveEvent,
  markEvent,
  findApprovedInvoiceTemplate,
...
export type ProcessInvoiceCreatedResult = {
  ok: true;
  duplicate: boolean;
  status: "reserved" | "completed" | "failed";
  messageId?: string;
...
/**
 * Orquesta invoice.created para Espacio Veloz. La reserva UNIQUE ocurre
 * antes de sendTemplate; un evento ya reservado/failed/completed no reenvía.
 */
export async function processInvoiceCreated(
  payload: InvoiceCreatedPayload,
  deps: InvoiceCreatedDeps = defaultInvoiceCreatedDeps
): Promise<ProcessInvoiceCreatedResult> {
  ...
  try {
    ...
    const template = await deps.findApprovedInvoiceTemplate(org.id);
    if (!template) {
      throw new WhmcsIntegrationError(
        409,
    ...
    const result = await deps.sendTemplate({
      ...
      variables: buildInvoiceTemplateVariables(payload),
      urlButtonSuffix: String(payload.invoiceId),
    });
